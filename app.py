...
GENERATED_PDF_DIR = resource_path("generated_pdfs")
MODEL = "llama-3.3-70b-versatile"
ENABLE_HANDOFF = True
GROQ_API_KEY = os.environ.get("GROQ_API_KEY")
if not GROQ_API_KEY:
    print("CRITICAL ERROR: GROQ_API_KEY not set.")
    sys.exit(1)
...
def save_generated_pdf_metadata(user_id, doc_type, title, business_name, filename):
    with get_db_connection() as conn:
        cur = conn.execute(
            "INSERT INTO pdf_history (user_id, doc_type, title, business_name, filename) VALUES (?, ?, ?, ?, ?)",
            (user_id, doc_type, title, business_name, filename),
        )
        return int(cur.lastrowid)

def build_pdf_bytes(title, content, business_name):
    title = title or "XenoBiz Document"
    business_name = business_name or "XenoBiz"
    buffer = io.BytesIO()
    ...
    c.drawString(margin, y, "XenoBiz")
    c.setFont("Helvetica", 11)
    c.drawString(margin, y - 24, business_name)
    ...
    c.drawString(margin, y, f"Date generated: {datetime.now().strftime('%B %d, %Y')}")
    y -= 28
    ...
    for paragraph in (content or "").split("\n"):
        lines = wrapper.wrap(paragraph.strip()) or [""]
        for line in lines:
            if text.getY() < margin + 40:
                ...
                y_new = height - margin
                c.setFont("Helvetica-Bold", 16)
                c.drawString(margin, y_new, title)
                c.setFont("Helvetica", 10)
                c.drawString(margin, y_new - 22, f"Date generated: {datetime.now().strftime('%B %d, %Y')}")
                text = c.beginText(margin, y_new - 48)
                text.setFont("Helvetica", 11)
                text.setLeading(16)
            text.textLine(line)
    c.drawText(text)
    ...
    c.drawCentredString(width / 2, footer_y, "Generated by XenoBiz AI Manager")
    c.showPage()
    c.save()
    buffer.seek(0)
    return buffer
...
def generate_pdf():
    ...
    title = data.get("title", "XenoBiz Document")
    business_name = data.get("business_name")

    allowed = {"job_description", "report", "recipe", "invoice", "summary"}
    if doc_type not in allowed:
        return jsonify({"error": "Invalid PDF type."}), 400
    if not content:
        return jsonify({"error": "PDF content is required."}), 400

    safe_title = re.sub(r"[^A-Za-z0-9 _-]", "", title).strip() or "XenoBiz Document"
    filename = f"{user_id}_{doc_type}_{datetime.now().strftime('%Y%m%d%H%M%S')}.pdf"
    file_path = os.path.join(GENERATED_PDF_DIR, filename)
    pdf_buffer = build_pdf_bytes(safe_title, content, business_name)

    with open(file_path, "wb") as f:
        f.write(pdf_buffer.read())
    pdf_buffer.seek(0)

    save_generated_pdf_metadata(user_id, doc_type, safe_title, business_name, filename)
...
@app.route("/generate/pdf/history", methods=["GET"])
@login_required
def pdf_history():
    user_id = get_current_user_id()
    with get_db_connection() as conn:
        rows = conn.execute(
            "SELECT id, doc_type, title, business_name, filename, generated_at FROM pdf_history WHERE user_id = ? ORDER BY generated_at DESC",
            (user_id,),
        ).fetchall()
    return jsonify([
        {
